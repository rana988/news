...
@section("content")
    <form class="row mb-3">
        ...
        <div class="col-2">
            <select name="bublish" class="form-control">
            <option value="">Any Status</option>
            <option {{request()->get('bublish')?'selected':''}} value="1">Active</option>
            <option {{request()->get('bublish')=='0'?'selected':''}} value="0">In Active</option>
        </select>
        </div>
        <div class="col-2">
            <select name="cat_id" class="form-control">
                ...
                @foreach($categories as $category)
                    <option {{'cat_id'==$category->id?"selected":""}} value='{{$category->id}}'>{{$category->id}} - {{$category->title}}</option>
                @endforeach
            </select>
        </div>
        <div class="col-2">
